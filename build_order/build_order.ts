...
function determineBuildOrder(projects: Array<string>, dependencies: Dependency[]): Array<string> | null {
        ...
        dependencyTree.forEach(node => {
            ...
            if (!built.includes(node[0])) {
                ...
                if (node[1].length === 0) {
                    ...
                    if (node[1].every(dep => built.includes(dep))) {
                        built.push(node[0])
                    } else {
                        built.forEach(builtDep => {
                            if (node[1].includes(builtDep)) {
                                let index = node[1].findIndex(a => a === builtDep);
